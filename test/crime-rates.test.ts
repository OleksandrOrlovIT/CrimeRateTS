...
describe('CrimeRateService', () => {
    ...
    it('should create a crime rate', async () => {
        ...
        sinon.assert.calledWith(httpServiceMock.get, `http://localhost:8080/api/city/${payload.cityId}`);
    ...
    it('should throw BadRequestException if city is not found', async () => {
        ...
        await expect(crimeRateService.createCrimeRate(payload)).to.be.rejectedWith(BadRequestException);
    });
